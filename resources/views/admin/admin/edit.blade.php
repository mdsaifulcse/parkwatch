    ...
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
        <div class="card">
            <div class="header">
                <h2>
                    {{ $title }}
                    @if($user->user_role==\App\User::PARKING_OWNER)
                ...
                {!! Form::open(['url' => 'admin/user/edit/', 'class' => 'form-validation frmValidation', 'files' => true]) !!}
                    ...
                    <div class="input-group">
                        ...
                        <div class="form-line  {{ $errors->has('user_role') ? 'error focused' : '' }}">
                            ...
                            @if(Auth::user()->user_role==\App\User::PARKING_OWNER)

                                {{ Form::select('user_role', [\App\User::PARKING_OWNER=>'Parking Owner',
                                ], $user->user_role, ['class'=>'form-control', 'id'=>'user_role']) }}

                            @else
                                {{ Form::select('user_role', $roles, $user->user_role, ['class'=>'form-control', 'id'=>'user_role']) }}
                            @endif
