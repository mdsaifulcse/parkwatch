        ...
        <div class="card"> 
            <div class="header">
                ...
                <ul class="header-dropdown m-r--5">  
                    <li>
                        <a href="{{ url('admin/states/create') }}" class="btn btn-sm btn-success waves-effect">
                            <i class="material-icons">add</i>
                            <span>{{ Lang::label('New State') }}</span>
                        </a>
                    </li>  
                    <li>
            ...
            <div class="body">
                {!! Form::open(['route' => ['states.update',$state->id], 'method'=>'POST', 'class' => 'form-validation frmValidation', 'files' => true]) !!}
                ...
                <div class="form-group">
                    <div class="form-line  {{ $errors->has('country_id') ? 'error focused' : '' }}">
                        {{ Form::select('country_id', $countries,  old('country_id',$state->country_id), ['class'=>'form-control', 'required'=>true,'id'=>'country_id', 'placeholder'=>Lang::label('Select Option')]) }}
                    </div>
                ...
                <div class="form-group">
                    <div class="form-line  {{ $errors->has('state_name') ? 'error focused' : '' }}">
                        <input name="state_name" type="text" id="state_name" class="form-control" required placeholder="{{ Lang::label(' Enter State Name Her') }}" value="{{ old('state_name',$state->state_name) }}">
                    </div>
                    ...
                    <div class="row">
                        <div class="col-sm-6">
                            <div class="switch {{ $errors->has('status') ? 'error focused' : '' }}">
                                <label>
                                    OFF<input name="status" type="checkbox" {{ ($state->status==\App\Models\State::PUBLISHED?'checked':null) }}>
