...
<div class="row clearfix">
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
        <div class="card">
            ...
            <div class="body">
                {!! Form::open(['route' => 'zones.store', 'method'=>'POST', 'class' => 'form-validation frmValidation', 'files' => true]) !!}
                ...
                <div class="form-group">
                    ...
                    <div id="loadState" class="form-line  {{ $errors->has('state_id') ? 'error focused' : '' }}">
                        {{ Form::select('state_id', [],  old('state_id'), ['class'=>'form-control', 'required'=>true,'id'=>'stateId', 'placeholder'=>Lang::label('First Select Country')]) }}
                    </div>
                ...
                <div class="form-group">
                    <div id="loadCity" class="form-line  {{ $errors->has('city_id') ? 'error focused' : '' }}">
                        {{ Form::select('city_id', [],  old('city_id'), ['class'=>'form-control', 'required'=>true,'id'=>'cityId', 'placeholder'=>Lang::label('Select Option')]) }}
                    </div>
                ...
                <div class="form-group">
                    <div class="form-line  {{ $errors->has('zone_name') ? 'error focused' : '' }}">
                        <input name="zone_name" type="text" id="zone_name" class="form-control" required placeholder="{{ Lang::label(' Enter Zone Name Here') }}" value="{{ old('zone_name') }}">
                    </div>
                        ...
                        <div class="row">
                            <div class="col-sm-6">
                                <div class="switch {{ $errors->has('status') ? 'error focused' : '' }}">
                                    <label>
                                        OFF<input name="status" type="checkbox" checked="" value="{{\App\Models\Zone::PUBLISHED}}">
