                ...
                <div class="row">
                    <div class="col-sm-12"> 
                        ...
                        <?php
                            $name='';
                            $clientId='';
                            if (!empty($price->client))
                            {
                                $name=$price->client->first_name.' '.$price->client->last_name;
                                $clientId=$price->client->id;
                            }
                        ?>

                            {{ Form::hidden('p_id', Request::segment(4) ) }}
                            {{ Form::hidden('f_id', Request::segment(5) ) }}
                            {{ Form::hidden('type', $price->type ) }}
                        ...
                        <div class="form-group">
                            <div class="form-line  {{ $errors->has('user_id') ? 'error focused' : '' }}">
                                ...
                                @if($price->type==\App\Models\Place::PARKINGOWNER)
                                    {{ Form::select('user_id', $users,  old('user_id',$price->user_id), ['id'=>'parkingOwnerId','class'=>'form-control', 'required'=>true, 'placeholder'=>'Select Parking Owner']) }}
                                @else
                        ...
                        <div class="form-group">
                            ...
                            <div id="loadParkingSpot" class="form-line  {{ $errors->has('place_id') ? 'error focused' : '' }}">
                                ...
                                {{ Form::select('place_id', $placeList,  old('place_id',$price->place_id), ['class'=>'form-control', 'id'=>'place_id','required'=>true, 'placeholder'=>Lang::label('Select Option')]) }}
                            </div>
                                ...
                                <div class="row priceContent">
                                    ...
                                    <div class="col-sm-2">
                                        <div class="form-line  {{ $errors->has('time') ? 'error focused' : '' }}">
                                            <input name="time[]" type="number" id="time" class="form-control" placeholder="{{ Lang::label('Time') }}" value="{{ $price->time }}" min="1" required>
                                        </div>
                                        @if ($errors->has('time'))
                                    ...
                                    <div class="col-sm-2">
                                        <div class="form-line  {{ $errors->has('unit') ? 'error focused' : '' }}">
                                            {{ Form::select('unit[]', $unitList, $price->unit , ['class'=>'form-control no-select', 'id'=>'unit', 'placeholder'=>Lang::label('Select Option'), 'required'=>'required']) }}
                                        </div>
                                        @if ($errors->has('unit'))
                                    ...
                                    <div class="col-sm-2">
                                        <div class="form-line  {{ $errors->has('price') ? 'error focused' : '' }}">
                                            <input name="price[]" type="text" id="price" class="form-control" placeholder="Price" value="{{ $price->price }}" required>
                                        </div>
                                        @if ($errors->has('price'))
