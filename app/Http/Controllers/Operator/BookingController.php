...
class BookingController extends Controller
{
    ...
    public function show(Request $request)
    {
        ...
        return view('operator.booking.list', compact('title','type', 'setting'));
    }
    ...
    { 
        ...
        if ($request->type == 'current')
        {
            ...
            ->whereIn('booking.place_id', Auth::user()->place_id())
            ->get();
        } 
        else if ($request->type == 'today')
        {
        ...
        return DataTables::of($bookings)
            ->addColumn('place_name', function($bookings){
                return "<a href=".url('operator/parking_zone/'.$bookings->place_id)." target=\"_blank\">".$bookings->place_name."</a>";

            }) 
            ->addColumn('client_id_no', function($bookings){
                return "<a href=".url('operator/client/profile/'.$bookings->client_id_no)." target=\"_blank\">".$bookings->client_id_no."</a>";
            }) 
    ...
    */ 
    public function form()
    {
        ...
        $placeList = Place::where('status', 1) 
            ->whereIn('id', Auth::user()->place_id())
            ->pluck('name', 'id');
            
        $vehicleTypeList = VehicleType::where('status', 1)->pluck('name', 'id');
        $priceList = ["" => "Please Select Parking Zone & Vehicle Type"]; 
 
        return view('operator.booking.form', compact('title', 'setting', 'priceList', 'vehicleTypeList', 'placeList'));
    }
